...
// ── Inline Normalizer (must be inlined — _shared bundler limitation for new files) ──
...
const _MO: Record<string,string> = {jan:'01',feb:'02',mar:'03',apr:'04',may:'05',jun:'06',jul:'07',aug:'08',sep:'09',oct:'10',nov:'11',dec:'12'};
...
function _nConf(c:any){if(!c||typeof c!=='string')return'medium';return _CM[c.toLowerCase()]||'medium';}
function _nDate(d:any){if(!d||typeof d!=='string')return d;const t=d.trim();if(!t)return null;if(/^\d{4}-\d{2}-\d{2}$/.test(t))return t;const s=t.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);if(s)return`${s[3]}-${s[1].padStart(2,'0')}-${s[2].padStart(2,'0')}`;const x=t.match(/^([A-Za-z]+)\s+(\d{1,2}),?\s+(\d{4})$/);if(x){const m=_MO[x[1].toLowerCase().slice(0,3)];if(m)return`${x[3]}-${m}-${x[2].padStart(2,'0')}`;}const e=t.match(/^(\d{1,2})\s+([A-Za-z]+)\s+(\d{4})$/);if(e){const m=_MO[e[2].toLowerCase().slice(0,3)];if(m)return`${e[3]}-${m}-${e[1].padStart(2,'0')}`;}return t;}
