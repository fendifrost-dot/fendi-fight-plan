...
function _dk(t:any){const cr=_nCred(t.creditor_name)||'';const ac=_nAcct(t.account_number)||'';const bu=Array.isArray(t.bureaus)&&t.bureaus.length>0?t.bureaus[0]:'unknown';const af=(ac&&ac!=='N/A'&&ac!=='UNEXTRACTABLE')?ac:`${_nBal(t.balance)||''}_${t.date_opened||''}`;return`${cr}|${af}|${bu}`;}
