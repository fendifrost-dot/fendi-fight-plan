...
function _nCred(n:any){if(!n||typeof n!=='string')return n;let c=n.replace(/\(\w{2,6}\)/g,'').toUpperCase().replace(/[\/\-_]+/g,' ').replace(/[^\w\s&'.]/g,' ').replace(/\s+/g,' ').trim();for(const k in _CMAP){if(c.includes(k)){c=_CMAP[k];break;}}return c;}
