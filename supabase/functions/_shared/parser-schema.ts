 ...
 * Canonical version for edge functions. Mirrored to src/lib/parser-schema.ts.
 * 
...
function validateDerogatoryAccount(account: any, index: number, entity = 'derogatory_accounts'): SchemaViolation[] {
  ...
  if (!isArrayOrUndefined(account.bureaus)) {
    violations.push({ entity, index, field: 'bureaus', reason: 'Must be array or undefined', critical: false });
  }
  if (!isArrayOrUndefined(account.derogatory_triggers)) {
    violations.push({ entity, index, field: 'derogatory_triggers', reason: 'Must be array or undefined', critical: false });
  }
  if (account.confidence && !['high', 'medium', 'low', 'incomplete'].includes(account.confidence)) {
    violations.push({ entity, index, field: 'confidence', reason: `Invalid confidence level: ${account.confidence}`, critical: false });
  }
  return violations;
}
...
export function validateSchema(result: any): SchemaValidationResult {
  ...
  if (result.derogatory_accounts && Array.isArray(result.derogatory_accounts)) {
    for (let i = 0; i < result.derogatory_accounts.length; i++) {
      const acctViolations = validateDerogatoryAccount(result.derogatory_accounts[i], i);
      violations.push(...acctViolations);
      const hasCritical = acctViolations.some(v => v.critical);
      if (hasCritical) {
        rejectedAccounts.push({ ...result.derogatory_accounts[i], _rejection_reasons: acctViolations.map(v => v.reason) });
      } else {
        validAccounts.push(result.derogatory_accounts[i]);
      }
    }
  }

  // Validate charge_offs with same rigor
  if (result.charge_offs && Array.isArray(result.charge_offs)) {
    for (let i = 0; i < result.charge_offs.length; i++) {
      violations.push(...validateDerogatoryAccount(result.charge_offs[i], i, 'charge_offs'));
    }
  }
